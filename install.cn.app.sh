...
echo cloning...
git clone https://github.com/odehDanOps/cloud-native-app-1/app-frontend-react
git clone https://github.com/odehDanOps/cloud-native-app-1/app-api-go

echo building frontend...
pushd ./app-frontend-react
cat > .env << EOF
REACT_APP_APIHOSTPORT=localhost:8080
EOF
yarn install
yarn build
docker build -t danops/frontend:v1 .
popd
...
docker run --name api --network cloudnativedemo --env MONGO_CONN_STR=mongodb://mongo:27017/langdb -d -p 8080:8080 danops/api:v1
docker run --name frontend --network cloudnativedemo -d -p 80:80 danops/frontend:v1
...
echo
echo
echo application now up and running...
echo http://localhost